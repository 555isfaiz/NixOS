...
local default_config = jdtls.document_config.default_config.cmd
local project_name = fn.fnamemodify(fn.getcwd(), ':p:h:t')

local workspace_dir = home_dir..".cache/jdtls/workspace/"..project_name

local get_cmd = function()
  local cmd = default_config
...
local get_jdp_javaagent = function()
  ...
  if jdp_versions ~= nil then
    ...
    jdp_versions:close()
    if next(lb_versions) ~= nil then
      local jdp_jar = fn.expand(string.format('%s%s/*.jar', jdp_dir, lb_versions[1]))
